...
getTransactions :: IO [[SqlValue]]
getTransactions = transactions where
  conn = connectPostgreSQL "host=Noah-MBP.local dbname=bank_accounts user=noahjones"
  q = "SELECT * FROM transactions;" :: String
  transactions = quickQuery conn q []

searchTransactions :: IO [[SqlValue]] -> Tx -> Bool
searchTransactions :: sql tx
...
-- switch above to checking database
